...
class mpSOM(nn.Module):
        ...
        if train:
            for epo in range(epoch):
                ...
                if epo % 10 == 0:
                    ...
                    self.layer1.save_result(os.path.join(self.save_path,'layer1','%d_layer1_result.png' % (epo)), (1, 7, 7))
            torch.save(self.layer1.state_dict(), self.layer1_pth)
        else:
    ...
    def run(self, dataloader, classifer):
        ...
        for idx, (X, Y) in enumerate(tqdm.tqdm(dataloader)):
            ...
            X5 = utils.location2matrix(location, self.layer5_conf['output_size'][0], self.layer5_conf['output_size'][1]).cuda()  # batch, h, w

            # classifier
            matrix = torch.cat([X1.flatten(1), X2.flatten(1), X3.flatten(1), X4.flatten(1), X5.flatten(1)], 1)
            b, _ = matrix.shape
            Y = Y.squeeze().cuda()
            with torch.no_grad():
...
class Classifier(torch.nn.Module):
    ...
    def test(self, sl, mpsom, test_loader):
        ...
        for idx, (X, Y) in enumerate(tqdm.tqdm(test_loader)):
            ...
            # Layer 5
            location = mpsom.layer5.map_multivects(inputs_restore)  # train som
            X5 = utils.location2matrix(location, mpsom.layer5_conf['output_size'][0],
                                       mpsom.layer5_conf['output_size'][1]).cuda()  # batch, h, w

            # classifier
            matrix = torch.cat(
                [X1.flatten(1), X2.flatten(1), X3.flatten(1), X4.flatten(1), X5.flatten(1)], 1)
            b, _ = matrix.shape
            Y = Y.squeeze().cuda()
            with torch.no_grad():
                pre = sl(matrix)
...
if __name__ == '__main__':
    ...
    config = conf.mpSOM_MNIST_5layers
    ...
    mpsom.train_layer3(epoch=100, train=True,device=device)
    mpsom.train_layer4(epoch=100, train=True,device=device)
    mpsom.train_layer5(epoch=100, train=True,device=device)
    mpsom.load()
